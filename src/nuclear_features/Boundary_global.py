...
def radii_features(binary_mask,centroid):
    ...
    feat= Radii_Features([np.min(dist_b_c),np.max(dist_b_c),np.median(dist_b_c),np.mean(dist_b_c),
           stats.mode(dist_b_c,axis = None)[0][0],
           np.percentile(dist_b_c,25),np.percentile(dist_b_c,75),
           np.std(dist_b_c),
           np.max(feret)])
...
def boundary_features(binary_image, centroids,angular_resolution=10,):
    ...
    all_features = pd.concat([calliper_features.reset_index(drop=True), radii_parameters], axis=1)
